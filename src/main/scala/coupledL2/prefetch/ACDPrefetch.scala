...
trait HasACDPParams extends HasPrefetcherHelper {
    def acdpParams = prefetchers.find {
        case p: ACDPParameters => true
        case _ => false
...
class PointerDataRecognition(implicit p: Parameters) extends ACDPModule {
  ...
  XSPerfAccumulate("pointerAddrValid", pointerAddrValid)
}
...
class AcdpReqBufferEntry(implicit p: Parameters) extends ACDPBundle {
  ...
  def toPrefetchReq(): PrefetchReq = {
    ...
    when(!hit_prefetched){
      switch(pfSource) {
        is (PfSource.ACDP.id.U)  { req.pfSource := MemReqSource.Prefetch2L2ACDP_d1.id.U }
        is (PfSource.ACDP_d1.id.U)  { req.pfSource := MemReqSource.Prefetch2L2ACDP_d2.id.U  }
      }
    }
    req
  }
...
class AdvanceContentDirecetdPrefetch(implicit p: Parameters) extends ACDPModule {
  ...
  val trainEnable = Mux(io.train.bits.pfsource === PfSource.ACDP_d2.id.U
  ...
  XSPerfAccumulate("acdp_req", io.req.fire)
  XSPerfAccumulate("acdp_train", io.train.fire)
