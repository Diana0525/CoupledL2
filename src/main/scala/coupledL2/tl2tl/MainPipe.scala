...
//  val a_need_replacement    = sinkA_req_s3 && !dirResult_s3.hit && meta_s3.state =/= INVALID // b and c do not need replacement
  ...
  io.prefetchTrain.foreach {
    train =>
      ...
      train.bits.train_for_acdp := (task_s3.valid && task_s3.bits.mshrTask && task_s3.bits.opcode === HintAck && task_s3.bits.dsWen) ||  
                                    need_data_a_s5
      train.bits.train_nofor_acdp := task_s3.valid && (((req_acquire_s3 || req_get_s3) && req_s3.needHint.getOrElse(false.B) &&
        (!dirResult_s3.hit || meta_s3.prefetch.get)) || req_s3.mergeA)
  }
